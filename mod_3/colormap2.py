...
def get_least_constraining_value(colors_dict, graph, node, index_num):
    ...
    return ordered_colors[index_num]


def assign_color(colors_dict, graph, node, index_num, trace):
    new_colors_dict = copy.deepcopy(colors_dict)

    node_colors = new_colors_dict[node]

    if isinstance(node_colors, str) or index_num > len(node_colors) - 1:
        if trace:
            pprint.pprint("BACKTRACKING: " + node + " because it ran out of viable options.")
        return colors_dict

    marked_color = get_least_constraining_value(colors_dict, graph, node, index_num)
    new_colors_dict[node] = marked_color

    for x in graph[node]:
        if x != node and len(new_colors_dict[x]) > 0 and marked_color in new_colors_dict[x] and len(
                new_colors_dict[x]) == 1:
            if trace:
                pprint.pprint("BACKTRACKING: " + node + " because it would have the same color as a neighbor.")
            return assign_color(colors_dict, graph, node, index_num + 1, trace)
...
def color_map(planar_map, colors, trace=False):
    ...
    while len(to_visit) > 0:
        ...
        if trace:
            pprint.pprint("VISITING: " + vertex)

        old_colors_dict = copy.deepcopy(colors_dict)
        colors_dict = assign_color(colors_dict, graph, vertex, i, trace)
        ...
        while colors_dict == old_colors_dict:
            ...
            colors_dict = assign_color(colors_dict, graph, vertex, child, trace)
        ...
        if vertex not in visited:
            ...
            for neighbor in graph[vertex]:
                if neighbor not in visited and neighbor not in to_visit:
                    if trace:
                        pprint.pprint("Added " + neighbor + " to the stack.")
    ...
    if trace:
        pprint.pprint("The colors generated (below)")
...
connecticut_colors = color_map(map, ["red", "blue", "green", "yellow"], trace=False)
